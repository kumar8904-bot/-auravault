...
public class MainActivity extends Activity {
    ...
    private void loadChat() {
        ...
        if (chat.isEmpty()) chat.add("A|Hey love… I’m here. Chat, call me, or create a new Anya moment. ♡");
    }
    ...
    private void showHome() {
        ...
        TextView greeting = text("Hey love… chat with me, call me, or open Perchance Studio to create new moments. ♡", 18, TEXT, false);
        ...
        boolean api = !prefs.getString("openrouter_key", "").trim().isEmpty();
        content.addView(section("v1.5 Perchance Edition"));
        ...
        content.addView(infoCard("Perchance Studio", "Your saved Perchance Anya preset now opens inside the app. Downloads are sent to Pictures/Anya when the site provides a normal download URL."));
    ...
    private void showChat() {
        ...
        renderThread(thread); content.addView(thread, mt(8));
        ...
        actions.addView(action("Send ♡", v -> {
            String s = input.getText().toString().trim();
            if (!s.isEmpty()) { input.setText(""); sendMessage(s, thread, false); }
        }), weight());
    ...
    private String openRouterReply(String key) throws Exception {
        ...
        String reply = out.getJSONArray("choices").getJSONObject(0).getJSONObject("message").optString("content", "").trim();
        if (reply.isEmpty()) throw new Exception("Empty AI reply");
        return reply;
    }
    ...
    private void showCall(boolean video) {
        ...
        content.addView(infoCard(video ? "Video status" : "Voice status", video ? "Voice conversation works; the portrait is still static. Animated lip-sync remains a later layer." : "Speech recognition and Android TTS are active."));
    }
    ...
    private void showVisualLab() {
        ...
        hero.addView(text("Perchance now opens inside Anya. Keep the same master face/reference image for consistency. Each website controls its own generation rules.", 13, MUTED, false));
        content.addView(hero);

        content.addView(section("Perchance Studio"));
        content.addView(action("✦ Open Perchance inside Anya", v -> openPerchance()), mt(8));
        content.addView(infoCard("Preset", "Your saved Perchance character link is preloaded. Use Perchance's own generate controls; normal image downloads are routed to Pictures/Anya."));

        content.addView(section("Other generators"));
        content.addView(secondary("Open FreeImageMaker", v -> openUrl(FREE_IMAGE_MAKER)), mt(8));
        content.addView(secondary("Open SnowArt", v -> openUrl(SNOWART)), mt(8));
    ...
    private void openPerchance() {
        Intent i = new Intent(this, PerchanceActivity.class);
        i.putExtra(PerchanceActivity.EXTRA_URL, PERCHANCE_ANYA);
        startActivity(i);
    }
    ...
    private void showSettings() {
        ...
        content.addView(action("Save AI settings", v -> {
            prefs.edit().putString("openrouter_key", key.getText().toString().trim()).putString("openrouter_model", model.getText().toString().trim().isEmpty() ? "openrouter/free" : model.getText().toString().trim()).apply();
            Toast.makeText(this, "AI settings saved locally.", Toast.LENGTH_SHORT).show();
        }), mt(8));
        ...
        content.addView(section("Visuals")); content.addView(infoCard("Perchance Edition", "In-app Perchance Studio + FreeImageMaker + SnowArt shortcuts"));
        ...
        content.addView(section("Build")); content.addView(infoCard("Version", "1.5.0 Perchance Edition"));
    }
